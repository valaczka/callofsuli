...
{
	qCDebug(lcApp).noquote() << QObject::tr("Create Application");
	...
	qCDebug(lcApp).noquote() << QObject::tr("Application created");
}
...
{
	qCDebug(lcApp).noquote() << QObject::tr("Load Qaterial %1.%2.%3").arg(qaterial::versionMajor()).arg(qaterial::versionMinor()).arg(qaterial::versionPatch());
