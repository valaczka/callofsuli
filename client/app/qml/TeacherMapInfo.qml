...
QSwipeContainer {
	...
	Connections {
		...
		function onMapDataReady(data) {
			...
			labelInfo.text = qsTr("Verzió: <b>%1</b><br>Méret: <b>%2</b><br>Utoljára módosítva: <b>%3</b><br>Eddigi játékok száma: <b>%4</b>").arg(data.version)
			.arg(cosClient.formattedDataSize(Number(data.dataSize)))
			.arg(data.lastModified)
			.arg(data.used)

			rptrGroup.model = data.binded

			btnDownload.visible = !data.downloaded

			if (data.mapReady) {
				rptrMission.model = data.missionList
			} else {
				rptrMission.model = []
			}
		}
