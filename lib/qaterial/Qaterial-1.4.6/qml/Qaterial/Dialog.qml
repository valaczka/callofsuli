...
T.Dialog
{
  ...
  Component.onCompleted: {
	  if (Qt.platform.os == "windows" || Qt.platform.os == "linux" || Qt.platform.os == "osx") {
		  for (var i=buttonBox.count-1; i>=0; --i) {
			  var b=buttonBox.itemAt(i)
			  if (b instanceof Qaterial.RawMaterialButton) {
				  if (b.DialogButtonBox.buttonRole === DialogButtonBox.AcceptRole ||
						  b.DialogButtonBox.buttonRole === DialogButtonBox.YesRole ||
						  b.DialogButtonBox.buttonRole === DialogButtonBox.ApplyRole) {
					  b.forceActiveFocus()
					  break
				  }
			  }
