...
private:
	...
	entt::entity generateDefender(const quint32 &idTag, const RpgStream::BaseDefenderObject::Type &type, const RpgStream::Team &team,
								  entt::entity defEnt, Defender *defender, const Chunk &chunk = {});
	...
	entt::entity defenderRender(entt::entity entity, DefenderObject *defender);
	void defenderRenderPulse(DefenderObject *defender, entt::entity target);
	entt::entity defenderRenderQuestionnaire(entt::entity entity, DefenderObject *defender, entt::entity target);
	entt::entity defenderRenderHpHealer(DefenderObject *defender, entt::entity target);

	void defenderLockUpdatePlayer(entt::entity player, const quint32 &lockId);
	...
	bool utility(Player *player, PlayerPrivate *priv, const RpgStream::PlayerConfig::Utility &type,
				 entt::entity target);
	bool utilityMissionary(Utility &utility);
	bool utilitySniper(Utility &utility);
	bool utilityBlock(Utility &utility, Player *player);
	bool utilityBoost(Utility &utility, Player *player);
	...
	void preRenderEventUnlock(const RpgStream::EventPlayer &event);
...
struct PlayerPrivate
{
	...
	std::unordered_map<quint32, entt::entity> defenderLock;
};
...
void RpgLogic::initializeStages()
{
	...
	scope.getCtx<RpgStream::GameConfig>()->setDuration(3*60*60);
...
/**
 * @brief RpgLogic::checkReadyToFinish
 * @return
 */

bool RpgLogic::checkReadyToFinish()
{
	return false;
}
...
entt::entity RpgLogic::playerAdd(const RpgStream::PlayerData &data, quint32 *idPtr, quint32 *tagIdPtr)
{
	...
	const Player &pp = m_registry.emplace<Player>(entity, std::move(playerData), data.team(), std::vector<Utility>{});
...
entt::entity RpgLogic::defenderAdd(const RpgStream::BaseDefenderObject::Type &type, const RpgStream::Team &team,
								   entt::entity defEnt, Defender *defender, const Chunk &chunk)
{
	return d->generateDefender(d->nextIdTag(RpgLogicPrivate::IdDefender), type, team, defEnt, defender, chunk);
}
...
/**
 * @brief RpgLogic::towerSet
 * @param tower
 * @param team
 * @param load
 */

void RpgLogic::towerSet(entt::entity tower, const RpgStream::Team &team, const quint32 &load)
{
	QMutexLocker locker(&m_mutex);

	Tower *t = m_registry.try_get<Tower>(tower);

	Q_ASSERT(t);

	RpgStream::TowerState &st = d->getEditableCurrentState<RpgStream::TowerState>(tower);

	st.setTeam(team);
	st.setLoad(std::min(100u, load));
	st.setActive(load >= CFG_TOWER_INACTIVE);
}



/**
 * @brief RpgLogic::chestAdd
 * @param chest
 * @return
 */

entt::entity RpgLogic::chestAdd(const Chest &chest)
{
	return d->chestAdd(chest);
}
...
RpgStream::Result RpgLogic::getResultByTeam(const RpgStream::Team &team)
{
	...
	for (auto e : m_registry.view<Player>()) {
		...
		float factorSinglePts = 1.;

		if (team == RpgStream::TeamNone && success) {
			factorSinglePts = (1. + state.heat() * CFG_RESULT_HEAT_RATIO);
		...
		if (team == RpgStream::TeamNone)
			r.result().setPts(state.ptsA() * factorSinglePts);
...
quint32 RpgLogicPrivate::playerDecreaseHp(entt::entity player, const quint32 &count)
{
	...
	if (const LockTag *pLock = q->m_registry.try_get<LockTag>(player)) {
		ELOG_DEBUG << "Clear player" << p.playerData.playerId() << "LockTag" << pLock->id;
		st.setLock(0);
									 ...
									 DefenderObject *object, RpgStream::DefenderState *state)
{
	...
	if (t == RpgStream::BaseDefenderObject::Questionnaire) {
		...
		q->m_registry.emplace<DefenderQuestionnaireObject>(entity);
	}
	...
	switch (t) {
		...
		case RpgStream::BaseDefenderObject::Electric:
			cfg = &cfgDefenderElectric.base;
			break;

		case RpgStream::BaseDefenderObject::Questionnaire:
			cfg = &cfgDefenderQuestionnaire.base;
			break;

		case RpgStream::BaseDefenderObject::HpHealer:
			cfg = &cfgDefenderHpHealer.base;
			break;
 ...
 * @param defender
 * @param countActionPtr
 */

entt::entity RpgLogicPrivate::defenderRender(entt::entity entity, DefenderObject *defender)
{
	Q_ASSERT(defender);
	...
	else if (defender->type == RpgStream::BaseDefenderObject::Questionnaire)
		return defenderRenderQuestionnaire(entity, defender, tg);
	else if (defender->type == RpgStream::BaseDefenderObject::HpHealer)
		return defenderRenderHpHealer(defender, tg);
...
/**
 * @brief RpgLogicPrivate::defenderRenderQuestionnaire
 * @param defender
 * @param target
 * @return
 */

entt::entity RpgLogicPrivate::defenderRenderQuestionnaire(entt::entity entity, DefenderObject *defender, entt::entity target)
{
	Q_ASSERT(defender);

	QMutexLocker locker(&q->m_mutex);

	if (q->m_registry.try_get<Npc>(target)) {
		defenderRenderPulse(defender, target);
		return target;
	}


	Player *p = q->m_registry.try_get<Player>(target);
	PlayerPrivate *pp = q->m_registry.try_get<PlayerPrivate>(target);

	const LockTag *pLock = q->m_registry.try_get<LockTag>(target);

	if (!p || pLock)
		return entt::null;

	if (q->m_registry.try_get<PreLockTag>(target))
		return entt::null;
	...
	for (auto e : q->m_registry.view<DefenderQuestionnaireObject>()) {
		const DefenderQuestionnaireObject &data = q->m_registry.get<DefenderQuestionnaireObject>(e);

		auto it = data.lastTick.find(target);

		if (it != data.lastTick.end() && (it->second + cfgDefenderQuestionnaire.base.repeaterDelay) > tick)
			return entt::null;
	}



	DefenderQuestionnaireObject *data = q->m_registry.try_get<DefenderQuestionnaireObject>(entity);

	Q_ASSERT(data);

	const quint32 lockId = playerLock(target, cfgDefenderQuestionnaire.force);

	if (lockId == 0)
		return entt::null;

	RpgStream::PlayerState &pst = getEditableCurrentState<RpgStream::PlayerState>(target);

	pst.setLock(lockId);

	data->lastTick[target] = tick;
	pp->defenderLock[lockId] = entity;
...
/**
 * @brief RpgLogicPrivate::defenderRenderHpHealer
 * @param defender
 * @param target
 */

entt::entity RpgLogicPrivate::defenderRenderHpHealer(DefenderObject *defender, entt::entity target)
{
	Q_ASSERT(defender);
	...
	pst.setHp(pst.hp() + cfgDefenderHpHealer.force);
...
/**
 * @brief RpgLogicPrivate::defenderLockUpdatePlayer
 * @param player
 * @param lockId
 */

void RpgLogicPrivate::defenderLockUpdatePlayer(entt::entity player, const quint32 &lockId)
{
	QMutexLocker locker(&q->m_mutex);

	// Register unlock time on defenders

	if (!q->m_registry.valid(player)) {
		ELOG_WARNING << "Player entity not found";
		return;
	}
	...
	const auto it = pp->defenderLock.find(lockId);

	if (it == pp->defenderLock.cend())
		return;

	if (!q->m_registry.valid(it->second))
		return;

	if (DefenderQuestionnaireObject *data = q->m_registry.try_get<DefenderQuestionnaireObject>(it->second)) {
		data->lastTick[player] = q->lastAuthTick()+1;
	}
}
...
quint32 RpgLogicPrivate::npcDecreaseHp(entt::entity npc, const quint32 &count)
{
	...
	quint32 nextTick = q->lastAuthTick();

	const RpgStream::GameConfig::Stage &stage = q->m_registry.ctx().get<RpgStream::GameConfig>().stage();

	switch (stage) {
		case RpgStream::GameConfig::StageWarmingUp:
			if (CFG_NPC_RESPAWN_STAGE_WU == 0)
				return nextTick;
			nextTick += CFG_NPC_RESPAWN_STAGE_WU;
			break;

		case RpgStream::GameConfig::StageMain:
			if (CFG_NPC_RESPAWN_STAGE_M == 0)
				return nextTick;
			nextTick += CFG_NPC_RESPAWN_STAGE_M;
			break;

		case RpgStream::GameConfig::StageLast:
			if (CFG_NPC_RESPAWN_STAGE_L == 0)
				return nextTick;
			nextTick += CFG_NPC_RESPAWN_STAGE_L;
			break;
	...
	const EventNpcCreate *orig = q->m_registry.try_get<EventNpcCreate>(npc);

	if (!orig) {
		ELOG_WARNING << "Missing NPC original create event";
		return nextHp;
	}

	if (orig->owner != entt::null) {
		return nextHp;
	}

	EventNpcCreate evc = *orig;

	evc.setTick(nextTick);
...
bool RpgLogicPrivate::utility(Player *player, PlayerPrivate *priv, const RpgStream::PlayerConfig::Utility &type, entt::entity target)
{
	...
	switch (type) {
		case RpgStream::PlayerConfig::UtilityMissionary:
			success = utilityMissionary(d);
			break;

		case RpgStream::PlayerConfig::UtilitySniper:
			success = utilitySniper(d);
			break;
		...
		case RpgStream::PlayerConfig::UtilityInvisible:
			success = utilityBlock(d, player);
			break;

		case RpgStream::PlayerConfig::UtilityBoostPoint:
			success = utilityBoost(d, player);
			break;

		case RpgStream::PlayerConfig::UtilityBoostAttackTower:
			success = true;
			break;
...
bool RpgLogicPrivate::utilityMissionary(Utility &utility)
{
	QMutexLocker locker(&q->m_mutex);

	if (!q->m_registry.valid(utility.target)) {
...
bool RpgLogicPrivate::utilitySniper(Utility &utility)
{
	QMutexLocker locker(&q->m_mutex);
	...
	if (npc) {
		RpgStream::NpcState &state = getEditableCurrentState<RpgStream::NpcState>(utility.target);
		state.setHp(0);
	...
	if (defender) {
		RpgStream::DefenderState &state = getEditableCurrentState<RpgStream::DefenderState>(utility.target);
...
/**
 * @brief RpgLogicPrivate::utilityBlock
 * @param utility
 * @param targetTeam
 * @return
 */

bool RpgLogicPrivate::utilityBlock(Utility &utility, Player *player)
{
	Q_ASSERT (player);

	QMutexLocker locker(&q->m_mutex);

	utility.targetTeam = RpgLogic::oppositeTeam(player->team);

	if (utility.type == RpgStream::PlayerConfig::UtilityBlockMpPick)
		utility.destroyAt = q->lastAuthTick() + cfgUtilityBlockMpPick.duration;
	else if (utility.type == RpgStream::PlayerConfig::UtilityBlockMpConvert)
		utility.destroyAt = q->lastAuthTick() + cfgUtilityBlockMpConvert.duration;
	else if (utility.type == RpgStream::PlayerConfig::UtilityBlockAttack)
		utility.destroyAt = q->lastAuthTick() + cfgUtilityBlockAttack.duration;
	else if (utility.type == RpgStream::PlayerConfig::UtilityInvisible)
		utility.destroyAt = q->lastAuthTick() + cfgUtilityInvisible.duration;

	ELOG_DEBUG << "Player" << player->playerData.playerId() << "start utility" << utility.type
			   << "target team" << utility.targetTeam
			   << "at" << q->lastAuthTick();

	player->activeUtility.emplace_back(utility);
...
/**
 * @brief RpgLogicPrivate::utilityBoost
 * @param utility
 * @return
 */

bool RpgLogicPrivate::utilityBoost(Utility &utility, Player *player)
{
	Q_ASSERT(player);

	QMutexLocker locker(&q->m_mutex);

	utility.targetTeam = player->team;

	if (utility.type == RpgStream::PlayerConfig::UtilityBoostPoint)
		utility.destroyAt = q->lastAuthTick() + cfgUtilityBoostPoint.duration;
											   ...
											   entt::entity defEnt, Defender *defender, const Chunk &chunk)
{
	...
	return generateDefender(nextIdTag(player, priv), type, player->team, defEnt, defender, chunk);
}
...
entt::entity RpgLogicPrivate::generateDefender(const quint32 &idTag, const RpgStream::BaseDefenderObject::Type &type, const RpgStream::Team &team,
											   entt::entity defEnt, Defender *defender, const Chunk &chunk)
{
	QMutexLocker locker(&q->m_mutex);
...
void RpgLogicPrivate::preRenderEventPlayer(entt::entity event)
{
	...
	switch (e.type()) {
		...
		case RpgStream::EventPlayer::EventUnlock:
			preRenderEventUnlock(e);
			break;

		case RpgStream::EventPlayer::EventRespawn:
		case RpgStream::EventPlayer::EventStreak:
...
void RpgLogicPrivate::preRenderEventPlayerMpPick(const RpgStream::EventPlayer &event)
{
	...
	for (auto e : q->m_registry.view<Player>()) {
		if (q->m_registry.get<Player>(e).hasUtilityTarget(RpgStream::PlayerConfig::UtilityBlockMpPick, p.team)) {
			ELOG_WARNING << "Player" << p.playerData.playerId() << "team" << p.team << "mp pick blocked" << st->tick();
			return;
		}
	}
...
void RpgLogicPrivate::preRenderEventTower(const RpgStream::EventPlayer &event)
{
	...
	for (auto e : q->m_registry.view<Player>()) {
		if (q->m_registry.get<Player>(e).hasUtilityTarget(RpgStream::PlayerConfig::UtilityBlockAttack, pp.team)) {
			ELOG_WARNING << "Player" << pp.playerData.playerId() << "team" << pp.team << "attack blocked" << st->tick();
			return;
		}
	}

	q->m_registry.emplace_or_replace<PreLockTag>(player);
	...
	if (st->utility() == RpgStream::PlayerConfig::UtilityBoostAttackTower && st->hasUtility()) {
		ELOG_INFO << "Player use boost utility" << st->tick();

		final.boost = true;

		EventUtility final2;
		final2.player = player;
		final2.target = tower;
		final2.type = st->utility();

		eventFinalStore(std::move(final2));
	}
...
void RpgLogicPrivate::preRenderEventAttackPlayer(const RpgStream::EventPlayer &event)
{
	...
	for (auto e : q->m_registry.view<Player>()) {
		if (q->m_registry.get<Player>(e).hasUtilityTarget(RpgStream::PlayerConfig::UtilityBlockAttack, pp.team)) {
			ELOG_WARNING << "Player" << pp.playerData.playerId() << "team" << pp.team << "attack blocked" << stPlayer.tick();
			return;
		}
	}
...
void RpgLogicPrivate::preRenderEventFailed(const RpgStream::EventPlayer &event)
{
	...
	ELOG_DEBUG << "Player" << event.tagId() << "failed, unlock with penalty" << penalty << "at" << q->lastAuthTick();

	playerUnlock(player, penalty);

	defenderLockUpdatePlayer(player, event.lockId());
}
...
void RpgLogicPrivate::preRenderEventChangeBullet(const RpgStream::EventPlayer &event)
{
	...
	for (auto e : q->m_registry.view<Player>()) {
		if (q->m_registry.get<Player>(e).hasUtilityTarget(RpgStream::PlayerConfig::UtilityBlockMpConvert, p.team)) {
			ELOG_WARNING << "Player" << p.playerData.playerId() << "team" << p.team << "mp convert blocked" << st.tick();
			return;
		}
	}
...
void RpgLogicPrivate::preRenderEventUseUtility(const RpgStream::EventPlayer &event)
{
	...
	if (st->utility() == RpgStream::PlayerConfig::UtilityBoostAttackTower) {
		ELOG_WARNING << "UtilityBoostAttackTower isn't callable";
		return;
	}
...
/**
 * @brief RpgLogicPrivate::preRenderEventUnlock
 * @param event
 */

void RpgLogicPrivate::preRenderEventUnlock(const RpgStream::EventPlayer &event)
{
	if (event.type() != RpgStream::EventPlayer::EventUnlock) {
		ELOG_ERROR << "Invalid event" << event.type();
		return;
	}

	if (event.lockId() == 0) {
		ELOG_WARNING << "Invalid lock id" << event.tagId();
		return;
	}
	...
	if (!pLock) {
		ELOG_WARNING << "Player isn't locked" << event.tagId();
		return;
	}

	if (pLock->id != event.lockId()) {
		ELOG_WARNING << "Player lockid mismatch" << event.tagId() << event.lockId() << pLock->id;
		return;
	}
...
void RpgLogicPrivate::preRenderEventNpcCreate(entt::entity ent)
{
	...
	// Save event for respawn
	EventNpcCreate ev = event;
	q->m_registry.emplace<EventNpcCreate>(e, std::move(ev));
...
void RpgLogicPrivate::preRenderEventNpcAttack(const RpgStream::EventNpc &event)
{
	...
	for (auto e : q->m_registry.view<Player>()) {
		...
		if ((npc->data.team() != RpgStream::TeamNone &&
			 p.hasUtilityTarget(RpgStream::PlayerConfig::UtilityBlockAttack, npc->data.team()))
				|| (npc->data.team() == RpgStream::TeamNone &&
					p.hasUtilityTarget(RpgStream::PlayerConfig::UtilityBlockAttack))) {
			ELOG_WARNING << "Npc" << npc->idTag << "team" << npc->data.team() << "attack blocked";
			return;
		}
	}
											  ...
											  Player *object, const RpgStream::EventNpc &event)
{
	...
	const bool inFog = npc->data.team() != RpgStream::TeamNone &&
										   checkInFog(cpv(stNpc.entityState().posXAsFloat(),
														  stNpc.entityState().posYAsFloat()),
													  RpgLogic::oppositeTeam(npc->data.team()));
	...
	final.npc = entity;
	final.target = target;
	final.withSuccess = stTarget.lock() == 0 &&
						!inFog &&
						!checkInFog(cpv(stTarget.entityState().posXAsFloat(),
										stTarget.entityState().posYAsFloat()),
									object->team);
...
void RpgLogicPrivate::preRenderDefenders()
{
	...
	for (auto e : q->m_registry.view<DefenderObject>()) {
		...
		const bool isHelper = (defender.type == RpgStream::BaseDefenderObject::HpHealer);
		...
		for (auto p : player) {
			...
			if (!isHelper && pp.team == defender.team)
				continue;
...
void RpgLogicPrivate::renderEvents(entt::entity ent, const std::vector<EventTower *> &list)
{
	...
	for (EventTower *e : list) {
		...
		if (e->boost) {
			ELOG_DEBUG << "Player" << player->playerData.playerId() << "boost attack tower" << q->lastAuthTick();

			playerIncreaseStreak(e->player, true);
...
*/
	...
	if (st.hp() > 0) {
		...
		if (t->type == RpgStream::BaseDefenderObject::Electric) {
			playerDecreaseHp(event->player, cfgDefenderElectric.force);
		}
	}

	if (t->type == RpgStream::BaseDefenderObject::Questionnaire) {
		if (DefenderQuestionnaireObject *data = q->m_registry.try_get<DefenderQuestionnaireObject>(event->target)) {
			data->lastTick[event->player] = q->lastAuthTick()+1;
		}
	}
...
void RpgLogicPrivate::renderEvents(EventChangeBullet *event)
{
	...
	ELOG_DEBUG << "Player" << p->playerData.playerId() << "load bullets at" << q->lastAuthTick();
...
void RpgLogicPrivate::renderEvents(EventNpcAttackDefender *event)
{
	...
	if (st.hp() > 0) {
		...
		if (t->type == RpgStream::BaseDefenderObject::Electric) {
			npcDecreaseHp(event->npc, cfgDefenderElectric.force);
		}
	}
...
void RpgLogicPrivate::renderEvents(EventNpcAttackPlayer *event)
{
	...
	PlayerPrivate *pp = q->m_registry.try_get<PlayerPrivate>(event->target);
	...
	const RpgStream::NpcState *nst = q->getCurrentState<RpgStream::NpcState>(event->npc);

	Q_ASSERT(nst);

	ELOG_DEBUG << "NPC" << p->idTag << "attacks player" << player->playerData.playerId() << "with success" << event->withSuccess << "at" << q->lastAuthTick();
	...
	if (event->withSuccess) {
		switch (p->data.type()) {
			...
			case RpgStream::NpcData::PlayerAttacker:
				if (st.hp() > 0) {
					const quint32 diff = std::min(st.hp(), std::max(1u, p->data.force()));
					playerDecreaseHp(event->target, diff);

					cpVect knockback = RpgLogic::addKnockbackImpulse(&st.entityState(), nst->entityState(),
																	 p->data.entity(), pp->toEntityConfig());
					if (!cpveql(knockback, cpvzero))
						q->m_registry.emplace_or_replace<KnockbackTag>(event->target, q->lastAuthTick());
...
void RpgLogicPrivate::renderEvents(EventDefenderStep *event)
{
	...
	entt::entity tg = defenderRender(event->defender, d);
...
void RpgLogicPrivate::renderEntityKnockbacks()
{
	...
	for (auto e : q->m_registry.view<KnockbackTag>()) {
...
/**
 * @brief RpgLogicPrivate::renderPlayerUtilities
 */

void RpgLogicPrivate::renderPlayerUtilities()
{
	QMutexLocker locker(&q->m_mutex);
	...
	std::vector<RpgStream::PlayerConfig::Utility> listA;
	std::vector<RpgStream::PlayerConfig::Utility> listB;

	for (auto e : q->m_registry.view<Player>()) {
		Player &p = q->m_registry.get<Player>(e);
		RpgStream::PlayerState &st = getEditableCurrentState<RpgStream::PlayerState>(e);

		std::vector<RpgStream::PlayerConfig::Utility> list;

		for (auto it = p.activeUtility.begin(); it != p.activeUtility.end(); ) {
			if (it->destroyAt > 0 && it->destroyAt <= tick) {
				ELOG_DEBUG << "Player" << p.playerData.playerId() << "remove utility" << it->type
						   << "target team" << it->targetTeam
						   << "at" << tick;

				it = p.activeUtility.erase(it);
			} else {
				list.emplace_back(it->type);

				if (it->targetTeam == RpgStream::TeamA)
					listA.emplace_back(it->type);
				else if (it->targetTeam == RpgStream::TeamB)
					listB.emplace_back(it->type);

				++it;
			}
		}

		st.setActiveUtilities(std::move(list));
	}


	RpgStream::GameState &state = q->m_registry.ctx().get<RpgStream::GameState>();

	state.setUtilitiesA(std::move(listA));
	state.setUtilitiesB(std::move(listB));
}
...
void RpgLogicPrivate::renderFinalTowers()
{
	...
	for (auto e : q->m_registry.view<Player>()) {
		...
		if (p.hasUtilityTarget(RpgStream::PlayerConfig::UtilityBoostPoint)) {
			if (p.team == RpgStream::TeamA)
				++multA;
			else if (p.team == RpgStream::TeamB)
				++multB;
	...
	for (auto e : view) {
		...
		if (st.team() == RpgStream::TeamA)
			mult = multA;
		else if (st.team() == RpgStream::TeamB)
			mult = multB;
...
void RpgLogicPrivate::renderFinalStage()
{
	...
	if (q->m_serverTick >= cfg.duration() || q->checkReadyToFinish()) {
...
void RpgLogicPrivate::removeDeleteTags()
{
	...
	auto view2 = q->m_registry.view<PreLockTag>();
	q->m_registry.remove<PreLockTag>(view2.begin(), view2.end());
}
...
void RpgLogicPrivate::controlUpdate(entt::entity entity, const RpgStream::ControlData &stream, Control *object,
									RpgStream::ControlState */*state*/)
{
	QMutexLocker locker(&q->m_mutex);
...
/**
 * @brief RpgLogicPrivate::chestAdd
 * @param chest
 * @return
 */

entt::entity RpgLogicPrivate::chestAdd(const Chest &chest)
{
	QMutexLocker locker(&q->m_mutex);
...
QPair<qint32, qint32> ChunkGrid::getChunk(const float &x, const float &y) const
{
	...
	pos.first = std::floor(std::clamp(x-(float) viewport.left(), 0.f, (float) viewport.width()) / chunkSize.width());
	pos.second = std::floor(std::clamp(y-(float) viewport.top(), 0.f, (float) viewport.height()) / chunkSize.height());
...
void PlayerPrivate::load(const RpgStream::PlayerConfig &cfg, RpgStream::PlayerData &dst, const bool &isHosted)
{
	if (!isHosted) {
		...
		const CfgPowerLevel pData = CfgPowerLevel::fromPlayerConfig(cfg).atLevel(power);
...
/**
 * @brief Player::hasUtilityTarget
 * @param utility
 * @param targetTeam
 * @return
 */

bool Player::hasUtilityTarget(const RpgStream::PlayerConfig::Utility &utility, const RpgStream::Team &targetTeam) const
{
	return activeUtility.cend() != std::find_if(activeUtility.cbegin(),
												activeUtility.cend(),
												[utility, targetTeam](const Utility &u) {
		return u.targetTeam == targetTeam && u.type == utility;
	});
}
...
bool Player::hasUtilityTarget(const RpgStream::PlayerConfig::Utility &utility) const
{
	return activeUtility.cend() != std::find_if(activeUtility.cbegin(),
												activeUtility.cend(),
												[utility](const Utility &u) {
		return u.type == utility;
	});
}
