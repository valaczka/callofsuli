...
#define CHECK_PASS(username, id)	\
	LAMBDA_SQL_ERROR("invalid id", \
	QueryBuilder::q(db).addQuery("SELECT id FROM studentgroup WHERE owner=") \
	.addValue(username).addQuery(" AND id=(SELECT groupid FROM pass WHERE id=").addValue(id).addQuery(")") \
	.execCheckExists());
...
#define CHECK_EXAM_STATE(username, id, state)	\
	LAMBDA_SQL_ERROR("invalid id", \
	QueryBuilder::q(db).addQuery("SELECT id FROM studentgroup WHERE owner=") \
	.addValue(username).addQuery(" AND id=(SELECT groupid FROM exam WHERE id=").addValue(id) \
	.addQuery(" AND ").addQuery(state).addQuery(")") \
	.execCheckExists());

#define CHECK_EXAM_CONTENT(username, id)	\
	LAMBDA_SQL_ERROR("invalid content id", \
	QueryBuilder::q(db).addQuery("SELECT id FROM studentgroup WHERE owner=") \
	.addValue(username).addQuery(" AND id=(SELECT groupid FROM exam " \
	"WHERE id=(SELECT examid FROM examContent WHERE id=").addValue(id).addQuery("))") \
	.execCheckExists());

#define CHECK_EXAM_CONTENT_STATE(username, id, state)	\
	LAMBDA_SQL_ERROR("invalid content id", \
	QueryBuilder::q(db).addQuery("SELECT id FROM studentgroup WHERE owner=") \
	.addValue(username).addQuery(" AND id=(SELECT groupid FROM exam " \
	"WHERE id=(SELECT examid FROM examContent WHERE id=").addValue(id) \
	.addQuery(" AND ").addQuery(state).addQuery("))") \
	.execCheckExists());
	...
	: AbstractAPI("teacher", handler, service)
{
	...
	server->route(path+"exam/<arg>/activate", QHttpServerRequest::Method::Post,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		return examActivate(*credential, {id});
	});

	server->route(path+"exam/activate", QHttpServerRequest::Method::Post,
				  [this](const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
		return examActivate(*credential, jsonObject->value(QStringLiteral("list")).toArray());
	});

	server->route(path+"exam/<arg>/inactivate", QHttpServerRequest::Method::Post,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		return examInactivate(*credential, {id});
	});

	server->route(path+"exam/inactivate", QHttpServerRequest::Method::Post,
				  [this](const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
		return examInactivate(*credential, jsonObject->value(QStringLiteral("list")).toArray());
	});

	server->route(path+"exam/<arg>/finish", QHttpServerRequest::Method::Post,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		return examFinish(*credential, {id});
	});

	server->route(path+"exam/finish", QHttpServerRequest::Method::Post,
				  [this](const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
	...
	server->route(path+"exam/answer/", QHttpServerRequest::Method::Post,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
	...
	server->route(path+"pass/", QHttpServerRequest::Method::Post|QHttpServerRequest::Method::Get,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		return pass(*credential, id, -1);
	});

	server->route(path+"pass/categories", QHttpServerRequest::Method::Post|QHttpServerRequest::Method::Get,
				  [this](const QHttpServerRequest &request){
		AUTHORIZE_API();
		return passCategories(*credential);
	});

	server->route(path+"group/<arg>/pass", QHttpServerRequest::Method::Post|QHttpServerRequest::Method::Get,
				  [this](const int &groupid, const QHttpServerRequest &request){
		AUTHORIZE_API();
		return pass(*credential, -1, groupid);
	});

	server->route(path+"group/<arg>/pass/create", QHttpServerRequest::Method::Post, [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
		return passCreate(*credential, id, *jsonObject);
	});

	server->route(path+"pass/<arg>/update", QHttpServerRequest::Method::Post, [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
		return passUpdate(*credential, id, *jsonObject);
	});

	server->route(path+"pass/<arg>/delete", QHttpServerRequest::Method::Post|QHttpServerRequest::Method::Get,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		return passDelete(*credential, QJsonArray{id});
	});

	server->route(path+"pass/delete", QHttpServerRequest::Method::Post, [this](const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
		return passDelete(*credential, jsonObject->value(QStringLiteral("list")).toArray());
	});

	server->route(path+"pass/<arg>/result", QHttpServerRequest::Method::Post|QHttpServerRequest::Method::Get,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
	...
	server->route(path+"pass/<arg>/create", QHttpServerRequest::Method::Post, [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
		return passItemCreate(*credential, id, *jsonObject);
	});

	server->route(path+"passItem/<arg>/update", QHttpServerRequest::Method::Post, [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
		return passItemUpdate(*credential, id, *jsonObject);
	});

	server->route(path+"passItem/<arg>/delete", QHttpServerRequest::Method::Post|QHttpServerRequest::Method::Get,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
	...
	server->route(path+"passItem/delete", QHttpServerRequest::Method::Post, [this](const QHttpServerRequest &request){
		AUTHORIZE_API();
		JSON_OBJECT_ASSERT();
		return passItemDelete(*credential, jsonObject->value(QStringLiteral("list")).toArray());
	});

	server->route(path+"passItem/<arg>/result", QHttpServerRequest::Method::Post|QHttpServerRequest::Method::Get,
				  [this](const int &id, const QHttpServerRequest &request){
		AUTHORIZE_API();
...
QHttpServerResponse TeacherAPI::campaignCreate(const Credential &credential, const int &group, const QJsonObject &json)
{
	...
	if (json.contains(QStringLiteral("starttime"))) {
		const qint64 &g = json.value(QStringLiteral("starttime")).toInteger();
	...
	if (json.contains(QStringLiteral("endtime"))) {
		const qint64 &g = json.value(QStringLiteral("endtime")).toInteger();
...
{
	LOG_CTRACE("client") << "Update exam" << id;
	...
	QueryBuilder q(db);
	q.addQuery("UPDATE exam SET ").setCombinedPlaceholder();

	if (json.contains(QStringLiteral("timestamp"))) {
		const qint64 &g = json.value(QStringLiteral("timestamp")).toInteger();
		q.addField("timestamp", g>0 ? QDateTime::fromSecsSinceEpoch(g).toUTC() : QVariant(QMetaType::fromType<int>()));
	}

	if (json.contains(QStringLiteral("mode")))
		q.addField("mode", json.value(QStringLiteral("mode")).toInt());
	...
	LOG_CDEBUG("client") << "Exam modified:" << id;
...
{
	LOG_CTRACE("client") << "Delete exam" << list;
	...
	LAMBDA_SQL_ASSERT(QueryBuilder::q(db).
					  addQuery("DELETE FROM exam WHERE id IN "
							   "(SELECT exam.id FROM exam LEFT JOIN studentgroup ON (studentgroup.id=exam.groupid) WHERE exam.id IN(")
					  .addList(list.toVariantList())
					  .addQuery(") AND studentgroup.owner=").addValue(credential.username())
					  .addQuery(")")
					  .exec());

	LOG_CDEBUG("client") << "Exams deleted:" << list;
...
QHttpServerResponse TeacherAPI::examResult(const Credential &credential, const int &id, const int &groupId)
{
	...
	if (idList.isEmpty()) {
		response = responseResult("list", QJsonArray());
...
/**
 * @brief TeacherAPI::examRemoveContent
 * @param credential
 * @param id
 * @param list
 * @return
...
QHttpServerResponse TeacherAPI::examAnswer(const Credential &credential, const int &id, const QJsonObject &json)
{
	LOG_CTRACE("client") << "Exam answer" << id;
	...
	LAMBDA_SQL_ASSERT_ROLLBACK(QueryBuilder::q(db)
							   .addQuery("INSERT OR REPLACE INTO examAnswer (")
							   .setFieldPlaceholder()
							   .addQuery(") VALUES (")
							   .setValuePlaceholder()
							   .addQuery(")")
...
/**
 * @brief TeacherAPI::examActivate
 * @param credential
 * @param list
 * @return
 */

QHttpServerResponse TeacherAPI::examActivate(const Credential &credential, const QJsonArray &list)
{
	LOG_CTRACE("client") << "Exam activate" << list;
...
/**
 * @brief TeacherAPI::examInactivate
 * @param credential
 * @param list
 * @return
 */

QHttpServerResponse TeacherAPI::examInactivate(const Credential &credential, const QJsonArray &list)
{
	LOG_CTRACE("client") << "Exam inactivate" << list;
...
/**
 * @brief TeacherAPI::examFinish
 * @param credential
 * @param list
 * @return
 */

QHttpServerResponse TeacherAPI::examFinish(const Credential &credential, const QJsonArray &list)
{
	LOG_CTRACE("client") << "Exam finish" << list;
...
QHttpServerResponse TeacherAPI::pass(const Credential &credential, const int &id, const int &groupId)
{
	LOG_CTRACE("client") << "Get pass" << id << "in group" << groupId;
	...
	q.addQuery("SELECT id, CAST(strftime('%s', starttime) AS INTEGER) AS starttime, groupid, "
			   "CAST(strftime('%s', endtime) AS INTEGER) AS endtime, title, grading, "
			   "IFNULL((SELECT childless FROM passHierarchy WHERE passid=pass.id LIMIT 1), TRUE) AS childless "
			   "FROM pass "
			   "WHERE groupid IN (SELECT id FROM studentgroup WHERE owner=").addValue(credential.username()).addQuery(")");
	...
	if (id <= 0) {
		const auto &categories = QueryBuilder::q(db)
								 .addQuery("SELECT id, description FROM passCategory WHERE username=")
								 .addValue(credential.username())
								 .execToJsonArray();

		LAMBDA_SQL_ASSERT(categories);
		...
		LAMBDA_SQL_ASSERT(items);

		QJsonObject r = list->at(0).toObject();
		r.insert(QStringLiteral("items"), *items);

		response = QHttpServerResponse(r);
	}
...
/**
 * @brief TeacherAPI::passCategories
 * @param credential
 * @return
 */

QHttpServerResponse TeacherAPI::passCategories(const Credential &credential)
{
	LOG_CTRACE("client") << "Get pass categories";
	...
	response = responseResult("list", *categories);
...
QHttpServerResponse TeacherAPI::passCreate(const Credential &credential, const int &group, const QJsonObject &json)
{
	LOG_CTRACE("client") << "Create pass in group" << group;
	...
	if (json.contains(QStringLiteral("title")))
		q.addField("title", json.value(QStringLiteral("title")).toString());

	if (json.contains(QStringLiteral("grading")))
		q.addField("grading", QString::fromUtf8(QJsonDocument(json.value(QStringLiteral("grading")).toObject()).toJson(QJsonDocument::Compact)));

	const auto &id = q.execInsertAsInt();

	LAMBDA_SQL_ASSERT(id);

	LOG_CDEBUG("client") << "Pass created:" << *id;
...
QHttpServerResponse TeacherAPI::passUpdate(const Credential &credential, const int &id, const QJsonObject &json)
{
	LOG_CTRACE("client") << "Pass exam" << id;
	...
	QueryBuilder q(db);
	q.addQuery("UPDATE pass SET ").setCombinedPlaceholder();
	...
	LOG_CDEBUG("client") << "Pass modified:" << id;
...
/**
 * @brief TeacherAPI::passDelete
 * @param credential
 * @param list
 * @return
 */

QHttpServerResponse TeacherAPI::passDelete(const Credential &credential, const QJsonArray &list)
{
	LOG_CTRACE("client") << "Delete pass" << list;
	...
	LAMBDA_SQL_ASSERT(QueryBuilder::q(db).
					  addQuery("DELETE FROM pass WHERE id IN "
							   "(SELECT pass.id FROM pass LEFT JOIN studentgroup ON (studentgroup.id=pass.groupid) WHERE pass.id IN(")
					  .addList(list.toVariantList())
					  .addQuery(") AND studentgroup.owner=").addValue(credential.username())
					  .addQuery(")")
					  .exec());

	LOG_CDEBUG("client") << "Passes deleted:" << list;
...
QHttpServerResponse TeacherAPI::passItemCreate(const Credential &credential, const int &pass, const QJsonObject &json)
{
	LOG_CTRACE("client") << "Create item in pass" << pass;

	if (pass <= 0)
		return responseError("invalid pass id");

	LAMBDA_THREAD_BEGIN(credential, pass, json);

	CHECK_PASS(credential.username(), pass);
	...
	if (json.contains(QStringLiteral("category"))) {
		if (const int &cid = json.value(QStringLiteral("category")).toInt(); cid <= 0) {
			const auto &id = QueryBuilder::q(db)
							 .addQuery("INSERT INTO passCategory(")
							 .setFieldPlaceholder()
							 .addQuery(") VALUES (")
							 .setValuePlaceholder()
							 .addQuery(")")
							 .addField("username", credential.username())
							 .addField("description", json.value(QStringLiteral("categoryDescription")).toString())
							 .execInsertAsInt();
	...
	QueryBuilder q(db);
	q.addQuery("INSERT INTO passItem(")
			.setFieldPlaceholder()
			.addQuery(") VALUES (")
			.setValuePlaceholder()
			.addQuery(")")
			.addField("passid", pass)
			;

	if (json.contains(QStringLiteral("includepass"))) {
		const int &g = json.value(QStringLiteral("includepass")).toInt();
		q.addField("includepass", g>0 ? g : QVariant(QMetaType::fromType<int>()));
	}

	if (json.contains(QStringLiteral("pts")))
		q.addField("pts", json.value(QStringLiteral("pts")).toInt());

	if (json.contains(QStringLiteral("extra")))
		q.addField("extra", json.value(QStringLiteral("extra")).toBool());

	if (json.contains(QStringLiteral("description")))
		q.addField("description", json.value(QStringLiteral("description")).toString());

	if (category > 0)
		q.addField("categoryid", category);
...
/**
 * @brief TeacherAPI::passItemUpdate
 * @param credential
 * @param id
 * @param json
 * @return
...
QHttpServerResponse TeacherAPI::passItemUpdate(const Credential &credential, const int &id, const QJsonObject &json)
{
	LOG_CTRACE("client") << "Update pass item" << id;
	...
	/// 0 = katetegória törlése (a passitemről)
	/// >0 = kategória módosíátsa
	///
	...
	if (category >= 0)
		q.addField("categoryid", category>0 ? category : QVariant(QMetaType::fromType<int>()));
	...
	LOG_CDEBUG("client") << "Pass item updated:" << id;
...
/**
 * @brief TeacherAPI::passItemDelete
 * @param credential
 * @param list
 * @return
 */
...
QHttpServerResponse TeacherAPI::passItemDelete(const Credential &credential, const QJsonArray &list)
{
	...
	LAMBDA_SQL_ASSERT(QueryBuilder::q(db).
					  addQuery("DELETE FROM passItem WHERE id IN "
							   "(SELECT passItem.id FROM passItem LEFT JOIN pass ON (pass.id=passItem.passid) "
							   "LEFT JOIN studentgroup ON (studentgroup.id=pass.groupid) WHERE passItem.id IN(")
					  .addList(list.toVariantList())
					  .addQuery(") AND studentgroup.owner=").addValue(credential.username())
					  .addQuery(")")
					  .exec());
...
/**
 * @brief TeacherAPI::passResult
 * @param credential
 * @param id
 * @return
 */

QHttpServerResponse TeacherAPI::passResult(const Credential &credential, const int &id)
{
	LOG_CTRACE("client") << "Pass result" << id;
...
/**
 * @brief TeacherAPI::passItemResult
 * @param credential
 * @param id
 * @return
 */

QHttpServerResponse TeacherAPI::passItemResult(const Credential &credential, const int &id)
{
	LOG_CTRACE("client") << "Pass item result" << id;
